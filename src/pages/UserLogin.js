...
import { Mail, Eye, EyeOff, Loader2, Lock, AlertCircle, X, Ticket } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-slate-50">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500">
        ...
        <div className="relative z-10 flex flex-col justify-center items-center text-white p-12 w-full animate-fadeIn">
          ...
          <p className="text-lg text-indigo-100 text-center max-w-md leading-relaxed">
            Problemlərinizi rəsmi şəkildə bildirin, status izləyin, admin ilə əlaqə saxlayın
          </p>
          <div className="mt-10 flex flex-wrap gap-3 justify-center">
            <span className="px-4 py-2 bg-white/15 backdrop-blur-sm rounded-full text-sm font-medium">📝 Müraciət</span>
            <span className="px-4 py-2 bg-white/15 backdrop-blur-sm rounded-full text-sm font-medium">📊 İzləmə</span>
            <span className="px-4 py-2 bg-white/15 backdrop-blur-sm rounded-full text-sm font-medium">💬 Cavab</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 lg:p-12">
        <div className="lg:hidden flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Müraciət Sistemi</h1>
        ...
        <div className="w-full max-w-md">
          <div className="bg-white/80 lg:backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-8 lg:p-10 animate-fadeIn">
            <div className="hidden lg:block mb-8">
              <h2 className="text-3xl font-bold text-slate-900">Xoş gəldiniz</h2>
              <p className="text-slate-500 mt-2">Hesabınıza daxil olmaq üçün məlumatları daxil edin</p>
            </div>
            ...
            {errorMessage && (
              <div role="alert" className="animate-slideDown mb-6 rounded-xl bg-red-50 border border-red-200 p-4 flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-red-800">Giriş uğursuz oldu</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input id="email" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="email@example.com" autoComplete="email" disabled={loading} className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all disabled:opacity-60" />
                </div>
              </div>

              <div>
                <label htmlFor="password" className="block text-sm font-semibold text-slate-700 mb-2">Şifrə</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 pointer-events-none" />
                  <input id="password" type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} placeholder="Şifrənizi daxil edin" autoComplete="current-password" disabled={loading} className="w-full pl-11 pr-12 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all disabled:opacity-60" />
              ...
              <div className="flex items-center justify-between text-sm">
                <Link to="/forgot-password" className="text-primary-600 hover:text-primary-700 font-medium">Şifrəni unutdum?</Link>
              </div>
            ...
            <div className="mt-6 text-center text-sm text-slate-600">
              Hesabınız yoxdur?{' '}
              <Link to="/register" className="text-primary-600 hover:text-primary-700 font-semibold">Qeydiyyatdan keçin</Link>
            </div>
          </div>

          <p className="text-center text-xs text-slate-400 mt-6">
            © 2026 Ticket Sistemi ·{' '}
            <Link to="/admin/login" className="hover:text-slate-600">Admin girişi</Link>
          </p>
        </div>
      </div>
    </div>
