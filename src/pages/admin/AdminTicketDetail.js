...
import { ArrowLeft, Loader2, Paperclip, Building2, User as UserIcon, Calendar, Trash2, Check } from 'lucide-react';
...
const STATUS_OPTIONS = [
  { value: 'pending', label: 'Gözləyən', color: 'bg-yellow-100 text-yellow-700' },
  { value: 'in_progress', label: 'İcradadır', color: 'bg-blue-100 text-blue-700' },
  { value: 'resolved', label: 'Həll Edildi', color: 'bg-green-100 text-green-700' },
];
...
const AdminTicketDetail = () => {
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      ...
      toast.success(newStatus === 'resolved' ? 'Müraciət həll edildi — istifadəçiyə email göndərildi' : 'Status yeniləndi');
      fetchTicket();
    } catch (err) {
  ...
  if (loading) {
    return <div className="p-12 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-amber-600" /></div>;
  }
  ...
  if (error || !ticket) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
        ...
        <Link to="/admin/tickets" className="text-amber-600 hover:text-amber-700 font-medium">
          ← Müraciətlərə qayıt
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Link to="/admin/tickets" className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-amber-600">
          <ArrowLeft className="w-4 h-4" />
          Müraciətlərə qayıt
        </Link>
        <button onClick={handleDelete} className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-medium rounded-lg border border-red-200">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h2 className="text-base font-semibold text-slate-900 mb-4">Status dəyişdir</h2>
        <div className="flex flex-wrap gap-2">
          {STATUS_OPTIONS.map((opt) => (
            ...
            >
              {ticket.status === opt.value && <Check className="inline w-4 h-4 mr-1" />}
              {opt.label}
            </button>
          ))}
        </div>
        <p className="text-xs text-slate-500 mt-3">
          <strong>Qeyd:</strong> "Həll Edildi" statusuna keçərkən istifadəçiyə avtomatik email göndərilir.
        </p>
        {ticket.resolvedAt && (
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
        💬 İstifadəçi ilə reply (comment thread) SP5-də əlavə olunacaq.
      </div>

      <CommentThread ticketId={ticket._id} api={adminCommentsAPI} currentRole="admin" />
    </div>
  );
};
